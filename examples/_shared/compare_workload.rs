//! Shared workload for the `compare` example and the `compare_profile`
//! gungraun benchmark.
//!
//! Holds every problem definition, the ~88 algorithm runner functions, and
//! the table-printing presentation layer. The example
//! (`examples/compare.rs`) is a thin shim that calls [`run_all`]; the
//! profiling benchmark (`benches/compare_profile.rs`) reuses the runner
//! functions directly so it exercises the exact same workload.
//!
//! This file is `#[path]`-included as a module by both, and lives under
//! `examples/_shared/` (no `main.rs`) so cargo does not auto-discover it
//! as an example.
...
pub const ZDT1_DIM: usize = 30;
pub const ZDT1_BUDGET: usize = 25_000;
...
pub const ZDT1_REFERENCE: [f64; 2] = [11.0, 11.0];

pub const RASTRIGIN_DIM: usize = 5;
pub const RASTRIGIN_BUDGET: usize = 50_000;

pub const DTLZ2_OBJECTIVES: usize = 3;
pub const DTLZ2_K: usize = 10;
pub const DTLZ2_DIM: usize = DTLZ2_OBJECTIVES + DTLZ2_K - 1; // 12
pub const DTLZ2_BUDGET: usize = 30_000;

pub const ROSENBROCK_DIM: usize = 5;
pub const ROSENBROCK_BUDGET: usize = 30_000;

pub const ACKLEY_DIM: usize = 5;
pub const ACKLEY_BUDGET: usize = 30_000;

pub const ZDT3_DIM: usize = 30;
pub const ZDT3_BUDGET: usize = 25_000;
pub const ZDT3_REFERENCE: [f64; 2] = [11.0, 11.0];

pub const DTLZ1_OBJECTIVES: usize = 3;
pub const DTLZ1_K: usize = 5;
pub const DTLZ1_DIM: usize = DTLZ1_OBJECTIVES + DTLZ1_K - 1;
pub const DTLZ1_BUDGET: usize = 30_000;
...
pub fn mean_l2_to_zdt1_front(front: &[Candidate<Vec<f64>>]) -> f64 {
    if front.is_empty() {
        return f64::INFINITY;
    }
...
pub fn zdt1_random(seed: u64) -> MoRun {
...
pub fn zdt1_paes(seed: u64) -> MoRun {
    let problem = Zdt1 { dim: ZDT1_DIM };
    let initializer = RealBounds::new(vec![(0.0, 1.0); ZDT1_DIM]);
...
pub fn zdt1_spea2(seed: u64) -> MoRun {
    let problem = Zdt1 { dim: ZDT1_DIM };
...
pub fn zdt1_nsga2(seed: u64) -> MoRun {
    let problem = Zdt1 { dim: ZDT1_DIM };
    let bounds = vec![(0.0, 1.0); ZDT1_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn zdt1_sms_emoa(seed: u64) -> MoRun {
    let problem = Zdt1 { dim: ZDT1_DIM };
    let bounds = vec![(0.0, 1.0); ZDT1_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn zdt1_hype(seed: u64) -> MoRun {
    let problem = Zdt1 { dim: ZDT1_DIM };
    let bounds = vec![(0.0, 1.0); ZDT1_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn zdt1_rvea(seed: u64) -> MoRun {
    let problem = Zdt1 { dim: ZDT1_DIM };
    let bounds = vec![(0.0, 1.0); ZDT1_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn zdt1_pesa2(seed: u64) -> MoRun {
    let problem = Zdt1 { dim: ZDT1_DIM };
    let bounds = vec![(0.0, 1.0); ZDT1_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn zdt1_epsilon_moea(seed: u64) -> MoRun {
    let problem = Zdt1 { dim: ZDT1_DIM };
    let bounds = vec![(0.0, 1.0); ZDT1_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn zdt1_mopso(seed: u64) -> MoRun {
    let problem = Zdt1 { dim: ZDT1_DIM };
...
pub fn zdt1_ibea(seed: u64) -> MoRun {
    let problem = Zdt1 { dim: ZDT1_DIM };
    let bounds = vec![(0.0, 1.0); ZDT1_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn zdt1_moead(seed: u64) -> MoRun {
    let problem = Zdt1 { dim: ZDT1_DIM };
    let bounds = vec![(0.0, 1.0); ZDT1_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn zdt1_nsga3(seed: u64) -> MoRun {
    let problem = Zdt1 { dim: ZDT1_DIM };
    let bounds = vec![(0.0, 1.0); ZDT1_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn dtlz2_random(seed: u64) -> MoRun {
...
pub fn dtlz2_nsga2(seed: u64) -> MoRun {
    let problem = dtlz2_problem();
...
pub fn dtlz2_spea2(seed: u64) -> MoRun {
    let problem = dtlz2_problem();
    let bounds = vec![(0.0, 1.0); DTLZ2_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn dtlz2_sms_emoa(seed: u64) -> MoRun {
    let problem = dtlz2_problem();
    let bounds = vec![(0.0, 1.0); DTLZ2_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn dtlz2_hype(seed: u64) -> MoRun {
    let problem = dtlz2_problem();
    let bounds = vec![(0.0, 1.0); DTLZ2_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn dtlz2_rvea(seed: u64) -> MoRun {
    let problem = dtlz2_problem();
    let bounds = vec![(0.0, 1.0); DTLZ2_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn dtlz2_pesa2(seed: u64) -> MoRun {
    let problem = dtlz2_problem();
    let bounds = vec![(0.0, 1.0); DTLZ2_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn dtlz2_epsilon_moea(seed: u64) -> MoRun {
    let problem = dtlz2_problem();
    let bounds = vec![(0.0, 1.0); DTLZ2_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn dtlz2_mopso(seed: u64) -> MoRun {
    let problem = dtlz2_problem();
...
pub fn dtlz2_ibea(seed: u64) -> MoRun {
    let problem = dtlz2_problem();
    let bounds = vec![(0.0, 1.0); DTLZ2_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn dtlz2_moead(seed: u64) -> MoRun {
    let problem = dtlz2_problem();
    let bounds = vec![(0.0, 1.0); DTLZ2_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn dtlz2_nsga3(seed: u64) -> MoRun {
    let problem = dtlz2_problem();
    let bounds = vec![(0.0, 1.0); DTLZ2_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn mean_distance_to_dtlz2_front(front: &[Candidate<Vec<f64>>]) -> f64 {
    if front.is_empty() {
        return f64::INFINITY;
    }
    let total: f64 = front
...
pub fn rastrigin_random(seed: u64) -> SoRun {
...
pub fn rastrigin_paes(seed: u64) -> SoRun {
    let problem = Rastrigin { dim: RASTRIGIN_DIM };
    let initializer = RealBounds::new(vec![(-5.12, 5.12); RASTRIGIN_DIM]);
...
pub fn rastrigin_nsga2(seed: u64) -> SoRun {
    let problem = Rastrigin { dim: RASTRIGIN_DIM };
...
pub fn rastrigin_de(seed: u64) -> SoRun {
    let problem = Rastrigin { dim: RASTRIGIN_DIM };
...
pub fn rastrigin_hill_climber(seed: u64) -> SoRun {
    let problem = Rastrigin { dim: RASTRIGIN_DIM };
    let initializer = RealBounds::new(vec![(-5.12, 5.12); RASTRIGIN_DIM]);
    let variation = BoundedGaussianMutation::new(0.3, vec![(-5.12, 5.12); RASTRIGIN_DIM]);
...
pub fn rastrigin_simulated_annealing(seed: u64) -> SoRun {
    let problem = Rastrigin { dim: RASTRIGIN_DIM };
    let initializer = RealBounds::new(vec![(-5.12, 5.12); RASTRIGIN_DIM]);
...
pub fn rastrigin_genetic_algorithm(seed: u64) -> SoRun {
    let problem = Rastrigin { dim: RASTRIGIN_DIM };
    let bounds = vec![(-5.12, 5.12); RASTRIGIN_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn rastrigin_particle_swarm(seed: u64) -> SoRun {
    let problem = Rastrigin { dim: RASTRIGIN_DIM };
    let bounds = RealBounds::new(vec![(-5.12, 5.12); RASTRIGIN_DIM]);
    let swarm = 40;
...
pub fn rastrigin_cma_es(seed: u64) -> SoRun {
    let problem = Rastrigin { dim: RASTRIGIN_DIM };
    let bounds = RealBounds::new(vec![(-5.12, 5.12); RASTRIGIN_DIM]);
    let pop = 16;
    let gens = RASTRIGIN_BUDGET / pop;
...
pub fn rastrigin_ipop_cma_es(seed: u64) -> SoRun {
    let problem = Rastrigin { dim: RASTRIGIN_DIM };
    let bounds = RealBounds::new(vec![(-5.12, 5.12); RASTRIGIN_DIM]);
    let pop = 16;
...
pub fn rastrigin_one_plus_one_es(seed: u64) -> SoRun {
    let problem = Rastrigin { dim: RASTRIGIN_DIM };
    let bounds = RealBounds::new(vec![(-5.12, 5.12); RASTRIGIN_DIM]);
...
pub fn rosenbrock_nelder_mead(_seed: u64) -> SoRun {
...
pub fn rosenbrock_one_plus_one_es(seed: u64) -> SoRun {
    let problem = rosenbrock_problem();
    let bounds = RealBounds::new(vec![(-5.0, 10.0); ROSENBROCK_DIM]);
    let config = OnePlusOneEsConfig {
...
pub fn rosenbrock_bo(seed: u64) -> SoRun {
    let problem = rosenbrock_problem();
    let bounds = RealBounds::new(vec![(-5.0, 10.0); ROSENBROCK_DIM]);
...
pub fn ackley_bo(seed: u64) -> SoRun {
...
pub fn rosenbrock_problem() -> Rosenbrock {
    Rosenbrock {
        dim: ROSENBROCK_DIM,
    }
}
pub fn ackley_problem() -> Ackley {
...
pub fn rosenbrock_de(seed: u64) -> SoRun {
    so_run_de!(
        rosenbrock_problem(),
        ROSENBROCK_DIM,
        -5.0,
...
pub fn rosenbrock_cma(seed: u64) -> SoRun {
    so_run_cma!(
        rosenbrock_problem(),
        ROSENBROCK_DIM,
        -5.0,
...
pub fn rosenbrock_pso(seed: u64) -> SoRun {
    so_run_pso!(
        rosenbrock_problem(),
        ROSENBROCK_DIM,
        -5.0,
...
pub fn rosenbrock_tlbo(seed: u64) -> SoRun {
    so_run_tlbo!(
        rosenbrock_problem(),
        ROSENBROCK_DIM,
        -5.0,
...
pub fn ackley_de(seed: u64) -> SoRun {
    so_run_de!(
        ackley_problem(),
        ACKLEY_DIM,
        -32.768,
...
pub fn ackley_cma(seed: u64) -> SoRun {
    so_run_cma!(
        ackley_problem(),
        ACKLEY_DIM,
        -32.768,
...
pub fn ackley_pso(seed: u64) -> SoRun {
    so_run_pso!(
        ackley_problem(),
        ACKLEY_DIM,
        -32.768,
...
pub fn ackley_tlbo(seed: u64) -> SoRun {
    so_run_tlbo!(
        ackley_problem(),
        ACKLEY_DIM,
        -32.768,
...
pub fn zdt3_problem() -> Zdt3 {
    Zdt3 { dim: ZDT3_DIM }
}

pub fn zdt3_nsga2(seed: u64) -> MoRun {
...
pub fn zdt3_moead(seed: u64) -> MoRun {
    let problem = zdt3_problem();
    let bounds = vec![(0.0, 1.0); ZDT3_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn zdt3_ibea(seed: u64) -> MoRun {
    let problem = zdt3_problem();
    let bounds = vec![(0.0, 1.0); ZDT3_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn zdt3_age_moea(seed: u64) -> MoRun {
    let problem = zdt3_problem();
    let bounds = vec![(0.0, 1.0); ZDT3_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn zdt3_knea(seed: u64) -> MoRun {
    let problem = zdt3_problem();
    let bounds = vec![(0.0, 1.0); ZDT3_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn dtlz1_nsga3(seed: u64) -> MoRun {
...
pub fn dtlz1_moead(seed: u64) -> MoRun {
    let problem = dtlz1_problem();
    let bounds = vec![(0.0, 1.0); DTLZ1_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn dtlz1_age_moea(seed: u64) -> MoRun {
    let problem = dtlz1_problem();
    let bounds = vec![(0.0, 1.0); DTLZ1_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn dtlz1_grea(seed: u64) -> MoRun {
    let problem = dtlz1_problem();
    let bounds = vec![(0.0, 1.0); DTLZ1_DIM];
    let initializer = RealBounds::new(bounds.clone());
    let variation = CompositeVariation {
...
pub fn mean_distance_to_dtlz1_front(front: &[Candidate<Vec<f64>>]) -> f64 {
    if front.is_empty() {
        return f64::INFINITY;
    }
    let total: f64 = front
...
pub fn run_rastrigin_comparison() {
    println!();
...
pub fn run_rosenbrock_comparison() {
    println!();
...
pub const TSP_CITIES: usize = 15;
pub const TSP_BUDGET: usize = 8_000;
pub const JSS_BUDGET: usize = 8_000;
pub const KNAPSACK_BUDGET: usize = 20_000;
...
pub fn tsp_random(seed: u64) -> SoRun {
...
pub fn tsp_hill_climber(seed: u64) -> SoRun {
    let problem = RingTsp::new();
...
pub fn tsp_simulated_annealing(seed: u64) -> SoRun {
    let problem = RingTsp::new();
...
pub fn tsp_tabu_search(seed: u64) -> SoRun {
    let problem = RingTsp::new();
...
pub fn tsp_genetic_algorithm(seed: u64) -> SoRun {
    let problem = RingTsp::new();
...
pub fn tsp_ant_colony(seed: u64) -> SoRun {
    let problem = RingTsp::new();
...
pub const JSS_JOBS: usize = 6;
pub const JSS_MACHINES: usize = 6;

/// FT06 routing — machine id of the k-th operation of job j.
pub const FT06_MACHINE: [[usize; JSS_MACHINES]; JSS_JOBS] = [
    [2, 0, 1, 3, 5, 4],
    [1, 2, 4, 5, 0, 3],
    [2, 3, 5, 0, 1, 4],
    [1, 0, 2, 3, 4, 5],
...
pub const FT06_TIME: [[f64; JSS_MACHINES]; JSS_JOBS] = [
...
pub const FT06_OPTIMAL_MAKESPAN: f64 = 55.0;
...
pub struct PrecedenceOrderCrossover;
...
pub fn pox_child(donor: &[usize], filler: &[usize], in_donor_set: &[bool]) -> Vec<usize> {
    let n = donor.len();
...
pub fn jss_initializer() -> ShuffledMultisetPermutation {
    ShuffledMultisetPermutation::new(vec![JSS_MACHINES; JSS_JOBS])
}

pub fn jss_random(seed: u64) -> SoRun {
    let mut opt = RandomSearch::new(
        RandomSearchConfig {
...
pub fn jss_hill_climber(seed: u64) -> SoRun {
    let mut opt = HillClimber::new(
        HillClimberConfig {
            iterations: JSS_BUDGET,
            seed,
...
pub fn jss_simulated_annealing(seed: u64) -> SoRun {
    let mut opt = SimulatedAnnealing::new(
        SimulatedAnnealingConfig {
            iterations: JSS_BUDGET,
            initial_temperature: 5.0,
...
pub fn jss_tabu_search(seed: u64) -> SoRun {
...
pub fn jss_genetic_algorithm(seed: u64) -> SoRun {
    let mut opt = GeneticAlgorithm::new(
        GeneticAlgorithmConfig {
            population_size: 80,
...
pub const KNAPSACK_N: usize = 30;
pub const KP_PROFIT_A: [f64; KNAPSACK_N] = [
...
pub const KP_PROFIT_B: [f64; KNAPSACK_N] = [
...
pub const KP_WEIGHT: [f64; KNAPSACK_N] = [
...
pub const KNAPSACK_REFERENCE: [f64; 2] = [0.0, 0.0];
...
#[derive(Debug, Clone, Copy, Default)]
pub struct OnePointCrossoverBool;
...
pub fn knap_binary_variation() -> CompositeVariation<OnePointCrossoverBool, BitFlipMutation> {
    CompositeVariation {
...
pub fn knap_run(front: &[Candidate<Vec<bool>>], problem: &BiKnapsack, wall_ms: u128) -> KnapRun {
    KnapRun {
...
pub fn knapsack_random(seed: u64) -> KnapRun {
...
pub fn knapsack_nsga2(seed: u64) -> KnapRun {
    let problem = BiKnapsack::new();
...
pub fn knapsack_spea2(seed: u64) -> KnapRun {
    let problem = BiKnapsack::new();
...
pub fn knapsack_nsga3(seed: u64) -> KnapRun {
    let problem = BiKnapsack::new();
...
pub fn knapsack_ibea(seed: u64) -> KnapRun {
    let problem = BiKnapsack::new();
...
pub fn run_knapsack_comparison() {
    println!();
    println!(
...
pub fn many_random(spec: ManySpec, seed: u64) -> MoRun {
...
pub fn many_nsga2(spec: ManySpec, seed: u64) -> MoRun {
    let problem = spec.problem();
    let mut opt = Nsga2::new(
        Nsga2Config {
...
pub fn many_nsga3(spec: ManySpec, seed: u64) -> MoRun {
    let problem = spec.problem();
    let mut opt = Nsga3::new(
        Nsga3Config {
            population_size: spec.population,
...
pub fn many_moead(spec: ManySpec, seed: u64) -> MoRun {
    let problem = spec.problem();
...
pub fn many_rvea(spec: ManySpec, seed: u64) -> MoRun {
    let problem = spec.problem();
...
pub fn many_grea(spec: ManySpec, seed: u64) -> MoRun {
    let problem = spec.problem();
...
pub fn many_ibea(spec: ManySpec, seed: u64) -> MoRun {
    let problem = spec.problem();
    let mut opt = Ibea::new(
        IbeaConfig {
            population_size: spec.population,
...
pub fn many_hype(spec: ManySpec, seed: u64) -> MoRun {
    let problem = spec.problem();
...
pub fn many_age_moea(spec: ManySpec, seed: u64) -> MoRun {
    let problem = spec.problem();
...
pub fn run_many_objective_comparison(title: &str, blurb: &[&str], spec: ManySpec) {
    println!();
    println!("== {title} ==");
    for line in blurb {
        println!("{line}");
